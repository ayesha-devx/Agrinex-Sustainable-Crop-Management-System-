...
# Construct the path to the CSV file safely
# Assumes your folder structure is backend/data/fertilizer.csv
...
def fertilizer_recommendation(crop_name, N, P, K):
    """
    Reads the data file, finds the ideal NPK for the given crop,
    compares it with input NPK, and returns advice.
    """
    try:
        # Load the CSV file
        ...
        # Find the row for the requested crop
        # We lowercase everything to make matching easier
        crop_row = df[df['Crop'].str.lower() == crop_name.lower()]
        
        if crop_row.empty:
            return "Crop not found in the database. Please try another crop."
        
        # Get ideal values (Assumes columns are named N, P, K)
        ...
        # Logic: Find the biggest discrepancy
        ...
        return "Error: Data file 'Fertilizer.csv' not found. Please check backend/data folder."
    except Exception as e:
        return f"Error processing fertilizer data: {str(e)}"
...
class NPKEstimator:
    def __init__(self):
        ...
        data_path = os.path.join(base_dir, 'data', "Nutrient_recommendation.csv")
        ...
        print(f"✅ Loaded CSV file from: {data_path}")
    ...
    def cropMapper(self):
        mapping = dict()
        mapped_crops_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mapped_crops.csv")

        with open(mapped_crops_path, "w") as fh:
            fh.write("Crops,Key\n")
